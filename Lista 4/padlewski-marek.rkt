...
(define (make-node elem heap-a heap-b)
  (let* ((left (if (>= (rank heap-a) (rank heap-b))
                   heap-a
                   heap-b))
         (right (if (>= (rank heap-b) (rank heap-a))
                    heap-b
                    heap-a))
         (this-rank (if (= (rank right) 0)
                        0
                        (inc (rank right)))))
    (list 'hnode elem this-rank left right)))
